...
\caption{Feature templates. $k$ is the boundary position, $t_k$ is the $k$ th POS tag (level 0), and $t^{l}_{i}$ is the $i$ th POS tag in the $l$-level POS tag sequence.}
\label{tbl:features}
\end{table}
...
distinguished by \textbf{POS level}, explained in detail in the next section. The specific
feature templates are shown in Table~\ref{tbl:features}.
...
classifier output along with the baseline parser in Table~\ref{tbl:wsj23}. The times reported are
...
\end{tabular}
\caption{Results of parsing WSJ section 23.}
\label{tbl:wsj23}
\end{table}
...
\section{\hspace{-1em}.\hspace{.5em} Derivation of equations in section \ref{sec:comp-saved}}
...
\section{\hspace{-1em}.\hspace{.5em} Detailed parse results}
